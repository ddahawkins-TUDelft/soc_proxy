...
def monte_carlo_tsa(
    df_source, date_lower_bound, date_upper_bound, feature_cols, n_rep_days, seed
):
    np.random.seed(seed)
    ...
    dayofyear_to_date = pd.date_range(
        start=f"{year}-01-01",
        periods=366
    ...
    assignment["timesteps"] = assignment["dayofyear"].apply(
        lambda d: dayofyear_to_date[d - 1].strftime("%Y-%m-%d")
    )
    assignment["PeriodNum"] = assignment["rep_dayofyear"].apply(
        lambda d: dayofyear_to_date[d - 1].strftime("%Y-%m-%d")
    )
