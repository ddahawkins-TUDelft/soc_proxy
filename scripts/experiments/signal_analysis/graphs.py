...
DF_CACHE_AllY = pd.read_csv(
    "SoC_proxy_TSA/data/signal_cache/signal_cache_master_5and10Y_WpSubset.csv"
)
DF_CACHE_10Y = pd.read_csv(
    "SoC_proxy_TSA/data/signal_cache/signal_cache_master_10Y_allWp.csv"
)
DF_CACHE_DIAGNOSTIC = pd.read_csv(
    "SoC_proxy_TSA/data/signal_cache/signal_cache_master.csv"
)

DF_COMBINED = pd.concat(
    [DF_CACHE_AllY, DF_CACHE_10Y], ignore_index=True
...
cmap_density = LinearSegmentedColormap.from_list(
    "white_to_ldes", ["#FFFFFF", COLOUR], N=256
)
...
def run_regression(df, window, use_deltas=False, compare_soc_not_ldes_cap=False):
    ...
    x1_field = (
        ...
        if use_deltas
        else f"win{window}d_tsa_proxy_nrmse_range"
    )
    x2_field = (
        f"win{window}d_proxy_delta_ref_vs_cem_delta_ref_nrmse_range"
        if use_deltas
        else f"win{window}d_proxy_ref_vs_cem_ref_nrmse_range"
    )
    y_field = (
        f"win{window}d_tsa_cem_nrmse_range"
        if compare_soc_not_ldes_cap
        else "ldes_cap_error_abs"
    )

    df = df[[y_field, x1_field, x2_field]].dropna()

    y_all = df[y_field].to_numpy()
    x_all = df[[x1_field, x2_field]].to_numpy()
...
def density_plot(
    ...
    (xmin, xmax), (ymin, ymax) = robust_limits(
        x_all, y_all, lo=0.5, hi=99.5, pad_frac=0.05
    )
    # xmin, ymin = 0, 0
    if y_min != "auto":
        ymin = y_min
    ...
    cf = ax.contourf(
        xx, yy, zz, levels=levels, cmap=cmap_density, vmin=zmin_draw, vmax=zmax
    )
    ...
    if df_scatter is not None:
        ...
        ax.scatter(x, y, s=18, alpha=0.9, linewidths=0.3, color=COLOUR_HIGHLIGHT)
...
def diagnostic_stats(x: pd.Series, y: pd.Series, show_prediction_interval=False):
    mask = np.isfinite(x) & np.isfinite(y)
    if mask.sum() < 10:
        return dict(r2=np.nan, beta=np.nan, p_beta=np.nan, rho=np.nan, p_rho=np.nan)
    ...
    *,
    ...
    if show_prediction_interval:
        ...
        print(
            half_width.mean(),
            half_width.median(),
            half_width.quantile(0.9),
            half_width.quantile(0.95),
        )
...
def pretty_print_fit(fit: dict, *, digits: int = 3) -> None:
    ...
    print(" ")
    print(
        f"Model: {fit['y_field']} ~ {', '.join(fit['x_fields'])}"
        + (f" + interactions({fit['interactions']})" if fit["interactions"] else "")
    )
    print(
        f"n={fit['n']}, R2={fit['r2']:.{digits}f}, adjR2={fit['r2_adj']:.{digits}f}, RMSE={fit['rmse']:.{digits}f}"
    )
    print("Coefficients:")
...
def moderation_table_Wp_by_M(
    ...
    try:
        df["M_bin"] = pd.qcut(
            df[m_field],
            q=m_bins,
            labels=[f"Q{i + 1}" for i in range(m_bins)],
            duplicates="drop",
        )
    except ValueError:
        df["M_bin"] = pd.cut(
            df[m_field], bins=m_bins, labels=[f"B{i + 1}" for i in range(m_bins)]
        )
...
def moderation_graph_density_quartiles_M_2x2(
    ...
    fig, axs = plt.subplots(
        2, 2, figsize=(fig_width * 2.4, fig_height * 2.2), sharex=True, sharey=True
    )
    bins = ["Q1", "Q2", "Q3", "Q4"]
...
def moderation_graph_slope_vs_M(
    ...
    def _plot_for_group(g, label):
        ...
        # line + error bars
        ax.errorbar(
            xs, slopes, yerr=1.96 * ses, fmt="o-", capsize=2, label=label, alpha=0.9
        )
...
def moderation_graph_density_hotspots_by_country(
    ...
    fig, axs = plt.subplots(
        1,
        len(countries),
        figsize=(fig_width * 1.3 * len(countries), fig_height * 1.2),
        sharex=False,
        sharey=False,
    )
...
def moderation_graph_density_best25_vs_rest75(
    ...
    def _rng(arr):
        return (
            (float(np.min(arr)), float(np.max(arr))) if len(arr) else (np.nan, np.nan)
        )
    ...
    fig, axs = plt.subplots(
        1, 2, figsize=(fig_width * 2.4, fig_height * 1.2), sharey=True, sharex=False
    )
...
def moderation_graph_density_Wp_bins_by_M(
    ...
    def _make_m_bins(d: pd.DataFrame):
        ...
        if mode == "best_vs_rest":
            ...
            d["M_bin"] = np.where(
                d[m_field] <= thr,
                f"Best {int(quantile_cut * 100)}%",
                f"Rest {int((1 - quantile_cut) * 100)}%",
            )
            labels = [
                f"Best {int(quantile_cut * 100)}%",
                f"Rest {int((1 - quantile_cut) * 100)}%",
        ...
        for gname, d2, _ in df_bins:
            ...
            if n_total > 0 and "M_bin" in d2.columns:
                vc = (
                    ...
                    .astype(str)
                    .value_counts()
                    .reindex([str(x) for x in col_labels])
                    .fillna(0)
                    .astype(int)
                )
...
def proxy_vs_proxyDeltas_regression(df_cache: pd.DataFrame):
    ...
    density_plot(
        ...
        df,
        y_field="e_tsa_cem_nrmse_range",
        x_field="e_proxy_ref_nrmse_range",
        free_intercept=True,
    )  # e_proxy_clu_nrmse_range  e_proxy_delta_clu_vs_cem_delta_clu_nrmse_range
    # ax1.set_ylim(0,0.4)
    # ax1.set_xlim(0,1)
    plt.show()
    ...
    density_plot(
        ...
        df,
        y_field="e_tsa_cem_nrmse_range",
        x_field="e_proxy_delta_ref_vs_cem_delta_ref_nrmse_range",
        free_intercept=True,
    )  # e_proxy_clu_nrmse_range  e_proxy_delta_clu_vs_cem_delta_clu_nrmse_range
    # ax2.set_ylim(0,0.4)
    # ax2.set_xlim(0,1)
...
def Q2_global_errors(df_cache: pd.DataFrame):
    ...
    XE = sm.add_constant(
        np.column_stack([x_l_a, x_l_t, x_d_a, x_d_t]), has_constant="add"
    )
    mE = sm.OLS(y, XE).fit()
    ...
    rows.append(
        _row(
            "Y ~ X_level_approx + X_level_tsa",
            mC,
            ["const", "X_level_approx", "X_level_tsa"],
        )
    )
    rows.append(
        _row(
            "Y ~ X_delta_approx + X_delta_tsa",
            mD,
            ["const", "X_delta_approx", "X_delta_tsa"],
        )
    )
    rows.append(
        _row(
            "Y ~ X_level_approx + X_level_tsa + X_delta_approx + X_delta_tsa",
            mE,
            ["const", "X_level_approx", "X_level_tsa", "X_delta_approx", "X_delta_tsa"],
        )
    )
...
def Q3_weight_quality_dependence(df_cache: pd.DataFrame):
    ...
    Q3a_plot_by_Wp_groups(
        df_cache=df_cache, y_field=y_field, x_field=x_field, free_intercept=True
    )
...
def Q3a_plot_by_Wp_groups(
    ...
    wp_groups = ["Wp = 0", "Wp ∈ {0.25, 0.5}", "Wp ∈ {0.75, 1.0}"]
    fig, axs = plt.subplots(
        3, 1, figsize=(fig_width * 1.5, fig_height * 3), sharex=True
    )
    ...
    x_label = (
        "Proxy-Delta TSA Error (nRMSE)"
        if x_field == "e_tsa_proxy_delta_nrmse_range"
        else ""
    )
...
def Q4a_diagnostics(df_cache: pd.DataFrame):
    ...
    metrics_approximation_error = [
        "e_proxy_clu_",
        "e_proxy_delta_clu_vs_cem_delta_clu_",
    ]
    window_metrics_approximation_error = [
        "d_proxy_clu_vs_cem_clu_",
        "d_proxy_delta_clu_vs_cem_delta_clu_",
    ]

    metrics_refProxy_vs_cluCem_error = [
        "e_proxy_ref_vs_cem_clu_",
        "e_proxy_delta_ref_vs_cem_delta_clu_",
    ]
    window_metrics_refProxy_vs_cluCem_error = [
        "d_proxy_ref_vs_cem_clu_",
        "d_proxy_delta_ref_vs_cem_delta_clu_",
    ]
    ...
    df["local_minus_global"] = (
        df["win180d_proxy_ref_vs_cem_clu_nmbe_range"]
        - df["win912d_proxy_ref_vs_cem_clu_nmbe_range"]
    )
    ...
    if df_Wp_gt_0.empty:
        raise ValueError(
            "No rows left after filtering to Wp>0. Cannot run Q4a_diagnostics."
        )
...
def Q4b_heatmap():
    ...
    index = pd.MultiIndex.from_tuples(
        data.keys(), names=["Metric family", "Error type"]
    )
...
def plot_hier_heatmap_clean(
    ...
    ax.set_position(
        [
            pos.x0 + left_label_pad * pos.width,
            pos.y0,
            pos.width * (1 - left_label_pad),
            pos.height,
        ]
    )
    ...
    for end in group_ends[:-1]:
        ...
        ax.hlines(
            y=y,
            xmin=-0.2,
            xmax=1.0,  # extend left into label margin
