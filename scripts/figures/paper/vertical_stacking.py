...
from matplotlib.offsetbox import OffsetImage, TextArea, HPacker, AnchoredOffsetbox
...
LOGS_F123_BY_CC = {
    cc: [Path(f"SoC_proxy_TSA/data/notes/log_{cc}_rerun.csv")] for cc in COUNTRIES
}
...
def resolve_reference_nc(
    dates: str, tvp: Optional[str], models_dir: Path
) -> Optional[Path]:
    ...
    if tvp_str and tvp_str.lower() not in {"", "nan", "none"}:
        ...
        candidates += [
            models_dir / f"standard_{y0}_{y1}_reference_{cc}.nc"
            if cc
            else models_dir / f"standard_{y0}_{y1}_reference.nc",
...
def get_capacities(m: calliope.Model):
    ...
    mask_drop = df_power_caps["techs"].isin(
    ...
    df_power_caps = df_power_caps[
        ...
        & (
            (
                (df_power_caps["techs"] == "electrolyser")
                & (df_power_caps["carriers"] == "hydrogen")
            )
            | (
                (df_power_caps["techs"] != "electrolyser")
                & (df_power_caps["carriers"] == "power")
            )
        )
    ]
...
def build_or_load_cem_cache(
    df_needed: pd.DataFrame, models_dir: Path, cache_path: Path
) -> pd.DataFrame:
    ...
    for _, row in df_needed.iterrows():
        ...
        try:
            power_caps_reference, energy_caps_reference = get_capacities(
                model_reference
            )
...
def build_or_load_runtime_cache(
    df_needed: pd.DataFrame, models_dir: Path, cache_path: Path
) -> pd.DataFrame:
    """
...
def fig4_error_vs_horizon_combined(df_cem: pd.DataFrame, path: Path) -> None:
    ...
    for colname, _, color, ax in metrics:
        data_W0 = [
            df[(df["h_bucket"] == h) & np.isclose(df["W_proxy"], 0.0)][colname]
            .dropna()
            .values
            for h in horizons
        ]
        data_W05 = [
            df[(df["h_bucket"] == h) & np.isclose(df["W_proxy"], 0.5)][colname]
            .dropna()
            .values
            for h in horizons
        ]
        data_W1 = [
            df[(df["h_bucket"] == h) & np.isclose(df["W_proxy"], 1)][colname]
            .dropna()
            .values
            for h in horizons
        ]
    ...
    legend_handles = [
        Line2D(
            ...
            [0],
            marker="s",
            color=COLOUR_GREY_MEAN,
            markerfacecolor="white",
            markeredgecolor=COLOUR_GREY_MEAN,
            linestyle="",
            label="$W_P = 0$ (hollow)",
        ),
        Line2D(
...
def fig5_runtime_vs_horizon_combined(df_runtime: pd.DataFrame, path: Path) -> None:
    ...
    df_mean = (
        df.groupby(["h_bucket", "number_reps"], dropna=False, as_index=False)[
            "runtime_min"
        ]
        .mean()
...
def add_flag_title(
    ...
    txt = TextArea(title, textprops=dict(size=fontsize, weight=fontweight, va="center"))
...
def fig2_box_by_reps(df_cem: pd.DataFrame, ax: plt.Axes, countrycode: str):
    ...
    data_ldes = [
        df_filter.loc[df_filter["number_reps"] == x, "ldes_error"].dropna().values
        for x in x_vals
    ]
    data_mac = [
        df_filter.loc[df_filter["number_reps"] == x, "macme"].dropna().values
        for x in x_vals
    ]
    ...
    for i, x in enumerate(x_vals):
        ...
        ax.scatter(
            xs,
            sub0["ldes_error"].values,
            s=fig_height * 4,
            marker="o",
            facecolors="white",
            edgecolors=COLOUR_LDES,
            linewidths=0.9,
            zorder=3,
        )
    ...
    legend_handles = [
        Line2D(
            ...
            marker=MARKER_LDES,
            color="none",
            markerfacecolor=COLOUR_LDES,
            markeredgecolor=COLOUR_LDES,
            label="LDES capacity error",
            markersize=6,
        ),
        Line2D(
            [0],
        ...
        Line2D(
            ...
            markeredgecolor=COLOUR_W0_EDGE,
            label="$W_P = 0$",
            markersize=6,
        ),
    ]
...
def fig3_box_by_proxy(df_cem: pd.DataFrame, ax: plt.Axes, countrycode: str):
    ...
    data_ldes = [
        df.loc[np.isclose(df["W_proxy"], x), "ldes_error"].dropna().values
        for x in x_levels
    ]
    data_mac = [
        df.loc[np.isclose(df["W_proxy"], x), "macme"].dropna().values for x in x_levels
    ]
...
def fig1_compound_heatmap(df_by_cc, path):
    fig, axes = plt.subplots(2, 1, figsize=(fig_width, fig_height * 2.25), sharex=True)

    # Reserve space: more room above top axis, and more gap between axes
    fig.subplots_adjust(left=0.12, right=0.95, bottom=0.10, top=0.92, hspace=0.32)
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list(
        "plasmaish_div_fixed", [(0.0, "#0d0887"), (0.5, "#ffffff"), (1.0, "#cc4778")]
    )
...
def fig2_compound_by_reps(df_by_cc, path):
    fig, axes = plt.subplots(
        2, 1, figsize=(fig_width, fig_height * 2.15), sharex=True, sharey=True
    )
...
def add_mask_outline(ax, mask_ok: np.ndarray, color: str, lw: float = 2.0):
    ...
    lc = LineCollection(
        segments, colors=[color], linewidths=lw, capstyle="round", joinstyle="round"
    )
...
def fig1_heatmap_10y_metric(
    df_cem: pd.DataFrame, ax: plt.Axes, countrycode: str, value_col: str
):
    """
    ...
    if value_col == "ldes_error":
        ...
        # cmap = plt.get_cmap("plasma")
        cmap = mcolors.LinearSegmentedColormap.from_list(
            "plasmaish_mono_fixed", [(0.0, "#ffffff"), (1.0, "#cc4778")]
        )
...
def fig2_strip_by_reps_metric(
    ...
    for i, reps in enumerate(x_vals):
        ...
        _line_channel(
            ax, pos_yes[i], y_all, color=lighten_hex(color, 0.25), line_width=1
        )
    ...
    legend_handles = [
        ...
        Line2D(
            ...
            marker="o",
            color="none",
            markerfacecolor=mcolors.to_rgba(color, 1),
            markeredgecolor=color,
            linestyle="",
            label="$W_P = 0.5$",
        ),
        ...
        Line2D(
            ...
            marker="o",
            color="none",
            markerfacecolor=lighten_hex(COLOUR_HIGHLIGHT, 0.2),
            markeredgecolor=COLOUR_HIGHLIGHT,
            linestyle="",
            label="Median",
        ),
    ]
...
def fig2_compound_strip_by_reps(
    df_by_cc, path: Path, value_col: str, color: str, signed: bool
):
    fig, axes = plt.subplots(
        2, 1, figsize=(fig_width, fig_height * 2.15), sharex=True, sharey=True
    )
    ...
    fig2_strip_by_reps_metric(
        df_by_cc["NL"], axes[0], "NL", value_col=value_col, color=color, signed=signed
    )
    fig2_strip_by_reps_metric(
        df_by_cc["BE"], axes[1], "BE", value_col=value_col, color=color, signed=signed
    )
...
def fig3_strip_by_proxy_metric(
    df_cem: pd.DataFrame,
    ax: plt.Axes,
    countrycode: str,
    value_col: str,
    ...
    for i, w in enumerate(x_levels):
        ...
        t_low, t_high = 45, 90
        y_rec = sub_w[
            (sub_w["number_reps"] >= t_low) & (sub_w["number_reps"] <= t_high)
        ][value_col].dropna()
        ...
        _scatter_channel(
            ...
            y_rec,
            color=color,
            facecolor=lighten_hex(color, 0.5),
            edgecolor=ec,
            jitter=jitter,
            rng=rng,
            s=s,
    ...
    legend_handles = [
        ...
        Line2D(
            ...
            marker="s",
            color="none",
            markerfacecolor=lighten_hex(color, 0.5),
            markeredgecolor=color,
            linestyle="",
            label="$k=[45,90]$",
        ),
...
def fig3_compound_strip_by_proxy(
    df_by_cc, path: Path, value_col: str, color: str, signed: bool
):
    fig, axes = plt.subplots(
        2, 1, figsize=(fig_width, fig_height * 2.15), sharex=True, sharey=True
...
def fig4_error_vs_horizon_combined_strip(df_cem: pd.DataFrame, path: Path) -> None:
    ...
    for i, h in enumerate(horizons):
        ...
        y0 = (
            sub[np.isclose(sub["W_proxy"], 0.0, atol=1e-9)]["ldes_error"]
            .dropna()
            .to_numpy()
        )
        y05 = (
            sub[np.isclose(sub["W_proxy"], 0.5, atol=1e-9)]["ldes_error"]
            .dropna()
            .to_numpy()
        )
        y1 = (
            sub[np.isclose(sub["W_proxy"], 1.0, atol=1e-9)]["ldes_error"]
            .dropna()
            .to_numpy()
        )
        ...
        _scatter_channel(
            ...
            y05,
            color=COLOUR_LDES,
            facecolor=lighten_hex(COLOUR_LDES),
            edgecolor=COLOUR_LDES,
            jitter=jitter,
            rng=rng,
            s=s,
        ...
        _scatter_channel(
            ...
            y1,
            color=COLOUR_LDES,
            facecolor=mcolors.to_rgba(COLOUR_LDES, 0.80),
            edgecolor=COLOUR_LDES,
            jitter=jitter,
            rng=rng,
            s=s,
    ...
    for i, h in enumerate(horizons):
        ...
        y0 = (
            sub[np.isclose(sub["W_proxy"], 0.0, atol=1e-9)]["macme"].dropna().to_numpy()
        )
        y05 = (
            sub[np.isclose(sub["W_proxy"], 0.5, atol=1e-9)]["macme"].dropna().to_numpy()
        )
        y1 = (
            sub[np.isclose(sub["W_proxy"], 1.0, atol=1e-9)]["macme"].dropna().to_numpy()
        )
        ...
        _scatter_channel(
            ...
            y05,
            color=COLOUR_MACME,
            facecolor=lighten_hex(COLOUR_MACME),
            edgecolor=COLOUR_MACME,
            jitter=jitter,
            rng=rng,
            s=s,
        ...
        _scatter_channel(
            ...
            y1,
            color=COLOUR_MACME,
            facecolor=mcolors.to_rgba(COLOUR_MACME, 0.80),
            edgecolor=COLOUR_MACME,
            jitter=jitter,
            rng=rng,
            s=s,
...
def fig6_pareto(
    ...
    for c_idx, cc in enumerate(countries):
        ...
        if x_all.size:
            ...
            pf = sub_cc.loc[m, ["abs_ldes_error", "speedup"]].sort_values(
                "abs_ldes_error"
            )
            if len(pf) >= 2:
                ax.plot(
        ...
        if cc != "ALL":
            ax.text(
                0.02, 0.98, cc, transform=ax.transAxes, ha="left", va="top", fontsize=9
            )
            ax2.text(
                0.02, 0.98, cc, transform=ax2.transAxes, ha="left", va="top", fontsize=9
            )
    ...
    k_handles = [
        Line2D(
            ...
            marker="o",
            color="none",
            markerfacecolor=k_to_colour[k],
            markeredgecolor=k_to_colour[k],
            linestyle="",
            label=f"k={int(k)}",
        )
        for k in ks
...
def main():
    ...
    df_cem_cache["ldes_error"] = -df_cem_cache[
        "ldes_error"
    ]  # so underinvestment is negative
    ...
    fig1_compound_heatmap_metric(
        df_f123_ready_by_cc,
        FIG1_HEATMAP_10Y_COMPOUND,
        value_col="ldes_error",
        cbar_label="LDES capacity error",
    )
    fig1_compound_heatmap_metric(
        df_f123_ready_by_cc,
        FIG1A_HEATMAP_MACME_10Y_COMPOUND,
        value_col="macme",
        cbar_label="MACME",
    )

    # Fig 2: strip plots by representative days (separate metrics)
    fig2_compound_strip_by_reps(
        df_f123_ready_by_cc,
        FIG2_LDES_STRIP_VS_REPS_COMPOUND,
        value_col="ldes_error",
        color=COLOUR_LDES,
        signed=True,
    )
    fig2_compound_strip_by_reps(
        df_f123_ready_by_cc,
        FIG2_MACME_STRIP_VS_REPS_COMPOUND,
        value_col="macme",
        color=COLOUR_MACME,
        signed=False,
    )
    ...
    fig3_compound_strip_by_proxy(
        df_f123_ready_by_cc,
        FIG3_LDES_STRIP_VS_PROXY_COMPOUND,
        value_col="ldes_error",
        color=COLOUR_LDES,
        signed=True,
    )
    fig3_compound_strip_by_proxy(
        df_f123_ready_by_cc,
        FIG3_MACME_STRIP_VS_PROXY_COMPOUND,
        value_col="macme",
        color=COLOUR_MACME,
        signed=False,
    )
    ...
    if not df_f45.empty:
        ...
        df_runtime_cache = build_or_load_runtime_cache(
            df_f45, MODELS_DIR, CACHE_RUNTIME
        )
        keys = df_f45[["dates", "tvp"]].drop_duplicates()
        df_run_ready = df_runtime_cache.merge(keys, on=["dates", "tvp"], how="inner")
        df_rt_rel = add_relative_runtime(df_runtime_cache)[
            ["id", "relative_runtime", "runtime_min", "runtime_ref_min"]
        ]
