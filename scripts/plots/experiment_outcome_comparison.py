"""Compare key CEM outcome metrics across experiment cases."""
...
def plot_experiment_outcome_comparison(
    case_ids: Sequence[str],
    *,
    results_dir: str | Path = "results",
    output_path: str | Path | None = None,
):
    """Plot key CEM outcome errors for selected experiment cases.

    The x-axis is the experiment name associated with each case ID.

    Panels show:

    * signed LDES capacity error;
    * signed total system cost error;
    * annualised-CAPEX-weighted MACME;
    * full-horizon clustered-vs-reference SoC level nRMSE.
    ...
    if not case_ids:
        raise ValueError(
            "At least one case_id must be supplied."
        )
...
def _experiment_labels(
    ...
    """Return experiment-name labels for requested cases."""
    ...
    if duplicates.any():
        raise RuntimeError(
            "parameters.parquet contains multiple "
            "experiment names for the same case_id."
        )
    ...
    if missing:
        raise KeyError(
            "Case IDs not found in "
            f"parameters.parquet: {missing}"
        )
...
def _investment_metric(
    ...
    """Extract one investment metric per requested case."""
...
def _signal_metric(
    ...
    *,
    ...
    """Extract one precisely defined signal metric per case."""

    mask = (
        metrics["case_id"].isin(case_ids)
        & (
            metrics["error_family"]
            == error_family
        )
        & (
            metrics["signal_type"]
...
def _to_case_values(
    ...
    *,
    ...
    if not duplicates.empty:
        raise RuntimeError(
            f"Metric {description!r} produced "
            "multiple rows for cases: "
    ...
    if missing:
        raise KeyError(
            f"Metric {description!r} is "
            "unavailable for case IDs: "
            f"{missing}"
        )
