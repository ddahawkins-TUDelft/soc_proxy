...
from soc_proxy.calliope.timeseries import (
    calliope_ts_to_pandas, extrapolate_ts_from_cluster_map
)
...
class ModelAdapter:
    ...
    def get_storage_soc_series(self, tech: str = "h2_salt_cavern") -> pd.Series:
        ...
        if self.kind == "clustered":
            ...
            df_intra = (m.results['storage'].fillna(0).to_series().dropna()
            ...
            df_inter = (m.results['storage_inter_cluster'].fillna(0).to_series().dropna()
            ...
            df = (m.results['storage'].fillna(0).to_series().dropna()
        ...
        if isinstance(self.raw, tsa_model):
            if self.kind == "clustered":
                df, _ = extrapolate_ts_from_cluster_map(
                    self.raw.paths['cluster_map'],
                    self.raw.paths['timeseries']
                )
            else:
    ...
    def get_capacities(self, power_techs, energy_techs=("h2_salt_cavern",)):
        ...
        m = self.calliope_model
        power_cap = (m.results['flow_cap'].fillna(0).to_series().dropna()
                     .to_frame('cap').reset_index())
        power_cap = power_cap[(power_cap['techs'].isin(power_techs)) &
                              (power_cap['carriers'] == 'power')]
        power_cap = (power_cap.groupby('techs')['cap'].sum()).sort_index()

        energy_cap = (m.results['storage_cap'].fillna(0).to_series()
                      .dropna().to_frame('cap').reset_index())
