...
def _error_wrapper(series_metric_name: str, reducer=mae) -> MetricFn:
    def fn(ctx: EvalContext):
        if ctx.ref is None:
            raise ValueError(
                f"Metric '{series_metric_name}_error' requires a reference."
            )
        ...
        ref_ctx = EvalContext(
            model=ctx.ref, ref=None, cache=ctx.cache, user_params=ctx.user_params
        )
...
METRICS.register(
    "socproxy_error_mae", _error_wrapper("soc_proxy"), needs_reference=True
)
...
def _magme(ctx: EvalContext) -> float:
    ...
    e_m = (
        pcap_m / (pcap_r + 1e-12)
    )  # pcap_m / pcap_m.sum() if pcap_m.sum() else pcap_m*0 #alternative looks at share mix error
    e_r = (
        pcap_r / (pcap_r + 1e-12)
    )  # pcap_r / pcap_r.sum() if pcap_r.sum() else pcap_r*0 #alternative looks at share mix error
