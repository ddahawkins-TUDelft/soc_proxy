...
from scripts.helpers.visualisation.metrics import (
    METRICS,
    EvalContext,
    _unwrap_metric_output,
)
...
def _is_tsa_model(obj) -> bool:
    ...
    return isinstance(obj, tsa_model) or (
        hasattr(obj, "calliope_model") and hasattr(obj, "tsa")
    )
...
def build_adapters(
    list_model_dict: List[Dict[str, Any]],
) -> Tuple[List[ModelAdapter], Optional[ModelAdapter]]:
    ...
    for md in list_model_dict:
        ...
        if _is_tsa_model(m):
            ...
            kind = getattr(m.tsa, "type", None) or m.calliope_model.params.get(
                "type", "unclustered"
            )
            params = {"name": name}
...
def visualise(
    ...
    if colour_field:
        ...
        probe_adapter = next(
            (a for a in adapters if a.kind != "reference"), adapters[0]
        )
        tmp_ctx = EvalContext(
            model=probe_adapter, ref=ref_adapter, cache={}, user_params={}
        )
    ...
    if scalar_colour:
        ...
        for adapter in adapters:
            ctx = EvalContext(
                model=adapter, ref=ref_adapter, cache=cache, user_params={}
            )
    ...
    for i, adapter in enumerate(adapters):
        ctx = EvalContext(
            model=adapter, ref=ref_adapter, cache=cache, user_params=user_params
        )
        ...
        if METRICS.needs_ref(y_field) and ref_adapter is None:
            raise ValueError(
                f"Metric '{y_field}' needs a reference but none was provided."
            )
    ...
    if show_tsa_internal_surplus_accumulation:
        ...
        if (
            m.tsa.type == "optimisation"
            and m.tsa.params["soc_proxy"]["optimisation_proxy_mode"] == "exogenous"
        ):
            df_internal_surpluses = m.soc_proxy.df
            df_internal_surpluses["accum_surplus"] = df_internal_surpluses[
                "surplus_LDES"
            ].cumsum()
            df_internal_surpluses["accum_surplus"] += df_internal_surpluses[
                "soc_proxy_LDES"
            ...
            plt.plot(
                df_internal_surpluses.index,
                df_internal_surpluses["accum_surplus"],
                label="TSA Internal SoC Proxy",
            )
        else:
            raise Exception(
                "show_tsa_internal_surplus_accumulation set to True, but tsa method is not an exogenous optimisation"
            )
    ...
    if scalar_colour and (sm is not None):
        ...
        cbar.set_label(
            f"{colour_field} (lower is better)"
            if DEFAULT_GOOD_IS_LOW
