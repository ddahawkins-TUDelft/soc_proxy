...
def compute_soc_proxy(
    surplus: np.ndarray, charging_eff: float, discharging_eff: float
) -> np.ndarray:
    """
...
def apply_temporal_rte_to_soc(
    ...
    """
    soc_proxy = compute_soc_proxy(
        df[surplus_col].values.astype(np.float64), charging_eff, discharging_eff
    )
    ...
    if check_cyclical:
        ...
        if abs(residual) > 1e4:
            print(
                f"[SoC Proxy] ⚠ Warning: SoC Proxy severely non-cyclical (end-start = {residual:.3e})"
            )
...
def allocate_circular_lifo(
    pos: np.ndarray, neg: np.ndarray, eta_ch: float, eta_dis: float
):
    """
...
def compute_volatility_margin(
    base_gen: np.ndarray, demand: np.ndarray, k=5e-2, min_m=2e-2, max_m=7e-2
):
    """
...
def find_min_feasible_curtailment(
    ...
    if f <= tol:
        ...
        while True:
            f_hi = evaluate_lost_load(c_hi, base_gen, demand, eta_ch, eta_dis)
            evals += 1
        ...
        while True:
            f_lo = evaluate_lost_load(c_lo, base_gen, demand, eta_ch, eta_dis)
            evals += 1
...
def decompose_surplus(
    ...
    if timestamp_col:
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            raise ValueError(
                "Timestamp column not provided and index is not a DatetimeIndex."
            )
    ...
    if timestep_hours == 0:
        raise ValueError(
            "Timestamps have zero time difference. Check the index or timestamp column."
        )
...
def generate_soc_proxy(
    ...
    soc_decomposition: dict = {"method": "fft_lowpass", "time_horizon_hours": 24},
    timestamp_col: str = None,
    ...
    if soc_decomposition["method"] not in supported_methods:
        raise ValueError(
            f"Method '{soc_decomposition['method']}' not supported. Choose from: {supported_methods}"
        )
    ...
    df[demand_field] = df[demand_field].astype(np.float64) - float(
        dispatchable_techs.get("known_dispatchable_capacity", 0.0)
    )

    # Capacity factors (unweighted) and weighted mean
    capacity_factors = {tech: float(df[tech].mean()) for tech in renewable_fields}
    capacity_factors["weighted_mean"] = float(
        sum(
    ...
    weighted_installed_capacity = float(
        df[demand_field].sum() / df["mean_capacity_factor"].sum()
    )

    # Build base generation (independent of curtailment) and demand vectors
    base_gen = (
        df["mean_capacity_factor"].to_numpy(np.float64) * weighted_installed_capacity
    )
    ...
    print(
        f"[SoC Proxy] Tuner: c*={c_star:.6f} lost={lost_final:.3e} | margin={margin:.3%} -> curtailment={curtailment_factor:.6f}"
    )
    ...
    installed_caps_nominal = {
        tech: float(weights[i] * weighted_installed_capacity)
        for i, tech in enumerate(renewable_fields)
    }
