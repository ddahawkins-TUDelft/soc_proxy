...
ENDO_BIAS_CONFIG = dict(
    ...
    coef_global_extreme=3.0,
    tau_global_extreme=5.0,
    ...
    if wP == 0.0:
        ...
        if proxy_window is None or proxy_window == "None":
            ...
            if not isinstance(feature_df.index, pd.DatetimeIndex):
                raise ValueError(
                    "feature_df.index must be a DatetimeIndex to use proxy_window."
                )
            start = pd.Timestamp(proxy_window[0])
            end = pd.Timestamp(proxy_window[1])
...
def _group_feature_columns(
    df_features: pd.DataFrame, preferred_features: List[str] | None = None
):
    ...
    if groups and all(len(sorted(v)) >= 24 for v in groups.values()):
        groups = {
            k: sorted([c for c in v if _HPAT.match(c)])[:24] for k, v in groups.items()
        }
    ...
    for base, cols in groups.items():
        ...
        if normalize == "minmax_signed":
            block = np.column_stack(
                [_minmax_signed(block[:, j]) for j in range(block.shape[1])]
            )
        elif normalize != "none":
        ...
        w = max(0.0, w)
        W_blocks.append(
            (np.sqrt(w) * np.ones(block.shape[1], dtype=float))
            if w > 0
...
def _build_a_and_soc_ref(
    reference_surplus: np.ndarray, eta_ch: float, eta_dis: float, cyc: bool = True
):
...
def _milp_tsa_core(
    *,
    ...
    surplus_daily_for_rows: dict[int, float]
    | None = None,  # row-id -> daily surplus (required if use_soc_term)
    # --- Objective mixing ---
    ...
    soc_bounds: tuple[float | None, float | None] = (
        None,
        None,
    ),  # (lo, hi) in SoC units; None keeps old auto-bounds
    # --- Solver knobs ---
    solver: str = "gurobi",
    MIPGap: float = 0.01,
    threads: int | None = 12,
    ...
    if candidates is None:
        ...
        def D_ij(i, j):
            return float(D[row_of_global[int(i)], j])
    ...
    if use_soc:
        assert (
            a is not None and soc_ref is not None and surplus_daily_for_rows is not None
        ...
        T_pos = sorted(
            set(
                [t for t in range(T) if active_mask[t]]
        ...
        if verbose:
            print(
                f"[TSA] SoC Objective term enabled. Active residuals: {len(T_pos)}/{T} "
    ...
    m = pyo.ConcreteModel()
    m.I = pyo.Set(
        initialize=row_ids, ordered=True
    )  # row ids (GLOBAL ids if restricted)
    ...
    m.D = pyo.Param(
        m.I,
        m.J,
        initialize={(i, j): D_ij(i, j) for i in row_ids for j in range(N)},
    ...
    # ORDO constraints
    m.assign_once = pyo.Constraint(
        m.J, rule=lambda _m, j: sum(_m.x[i, j] for i in _m.I) == 1
    )
    ...
    if use_soc:
        ...
        if soc_bounds != (None, None):
            lo = (
                float(soc_bounds[0])
                if soc_bounds[0] is not None
                else float(np.min(soc_ref)) - 1e-9
            )
            hi = (
                float(soc_bounds[1])
                if soc_bounds[1] is not None
                else float(np.max(soc_ref)) + 1e-9
            )
        else:
        ...
        def _z_bounds(_m, t):
            return (0.0, (hi - lo) + abs(float(soc_ref[int(t)])))

        m.socdiff = pyo.Var(m.Tz)
        m.z = pyo.Var(m.Tz, domain=pyo.NonNegativeReals, bounds=_z_bounds)
        m.soc_def = pyo.Constraint(
            m.Tz, rule=lambda _m, t: _m.socdiff[t] == soc_ref[int(t)] - _m.soc_raw[t]
        )
    ...
    if candidates is None:
        ...
        dist_scale = max(
            ...
            float(
                np.max(
                    D[[{j: i for i, j in enumerate(row_ids)}[i] for i in row_ids], :]
                )
            ),
        )
    ...
    m.dist_term_scaled = pyo.Expression(
        rule=lambda _m: (
            sum(_m.x[i, j] * _m.D[i, j] for i in _m.I for j in _m.J) / dist_scale
        )
    )
    if verbose:
    ...
    if use_soc:
        ...
        m.prox_term_scaled = pyo.Expression(
            rule=lambda _m: (
                (sum(b_for_Tz[int(t)] * _m.z[t] for t in _m.Tz) / prox_scale)
                if len(T_pos)
                else 0.0
            )
        )
        m.obj = pyo.Objective(
            rule=lambda _m: (
                (1 - lambda_soc) * _m.dist_term_scaled
                + lambda_soc * _m.prox_term_scaled
            ),
            sense=pyo.minimize,
        )
    else:
    ...
    if solver == "gurobi":
        ...
        if threads is not None:
            opt.set_gurobi_param("Threads", int(threads))
        ...
        res = pyo.SolverFactory(solver).solve(
            m, tee=verbose, options={"MIPGap": MIPGap, "TimeLimit": int(timelimit)}
        )
    ...
    assignments = {
        int(j): max([int(i) for i in m.I], key=lambda i: pyo.value(m.x[i, j]))
        for j in m.J
    }
    ...
    try:
        ...
        if use_soc:
            ...
            print(
                f"[TSA] [Obj.] total={obj_val:.6g} | dist={dist_contrib:.6g} | prox={prox_contrib:.6g} "
                f"| shares: dist={share_dist:.3f}, prox={share_prox:.3f}"
            )
            breakdown = dict(
                obj_total=obj_val,
                dist_scaled=dist_scaled,
                prox_scaled=prox_scaled,
            ...
            print(
                f"[TSA] [Obj.] total={obj_val:.6g} | dist={dist_scaled:.6g} (SoC off)"
            )
            breakdown = dict(
                obj_total=obj_val,
                dist_scaled=dist_scaled,
...
def solve_ordo_with_endogenous_soc(
    *,
    ...
    verbose=True,
    surplus_by_day=None,
    use_endogenous_biases=False,
    timelimit=1200,
):
    D = _ordo_cost_matrix_from_features(
        df_features, preferred_features, feature_weights, normalize
    )
    N = len(df_features)
    ...
    biases = (
        (generate_endogenous_biases(surplus_by_day.cumsum(), params=ENDO_BIAS_CONFIG))
...
def solve_tsa(
    *,
    ...
    surplus_by_day: np.ndarray
    | None = None,  # shape (N,), daily net surplus (same order as df_features)
    reference_surplus: np.ndarray
    | None = None,  # optional; if None, uses surplus_by_day as the reference
    eta_ch: float = 1.0,
    eta_dis: float = 1.0,  # efficiencies if you re-enable them in _build_a_and_soc_ref
    ...
    ref_surplus = (
        reference_surplus.reshape(-1) / s_scale
        if reference_surplus is not None
        else S_scaled
    )
    a_vec, soc_ref = _build_a_and_soc_ref(
        ref_surplus, eta_ch=eta_ch, eta_dis=eta_dis, cyc=True
    )
    ...
    if use_endogenous_biases:
        ...
        if biases_params:
            params.update(biases_params)
        # Typical input for biases: a SoC-like proxy; using cumulative surplus here
        biases = generate_endogenous_biases(
            pd.Series(np.cumsum(S_scaled)), params=params
        )
...
def rebuild_from_assignments(
    df_features: pd.DataFrame, rep_for_day: np.ndarray
) -> pd.DataFrame:
...
def save_milp_result_to_cluster_map(
    result: dict, dates_index: pd.DatetimeIndex, output_path: str
):
    ...
    day_to_rep = {
        daily_dates[j]: daily_dates[result["assignments"][j]]
        for j in result["assignments"]
    }
    mapping_df = pd.DataFrame.from_dict(
        day_to_rep, orient="index", columns=["PeriodNum"]
    )
