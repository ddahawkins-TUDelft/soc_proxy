...
def cluster_tsa_with_extremes(
    ...
    if soc_features:
        if soc_proxy_dict.get("use_soc_proxy"):
            ...
            if base_proxy not in weightDict:
                raise KeyError(
                    f"Proxy column '{base_proxy}' must have a base weight in weightDict."
                )
    ...
    if soft_prune and pool_grew:
        ...
        if kept_extremes:
            print(
                ">>> TSA: Kept extremes:",
                ", ".join(str(d.date()) for d in kept_extremes),
            )
        if dropped_extremes:
            print(
                ">>> TSA: Dropped extremes:",
                ", ".join(str(d.date()) for d in dropped_extremes),
            )
        ...
        print(
            f"[TSA] Using k + E representatives = {len(chosen_dates)} "
    ...
    if matched_indices.index.duplicated().any():
        ...
        print(
            f"[TSA] Warning: dropping {dup_count} duplicated timesteps in matched_indices."
        )
        matched_indices = matched_indices[
            ~matched_indices.index.duplicated(keep="first")
        ]
    ...
    if soc_proxy_dict.get("use_soc_proxy"):
        ...
        if added_cols_all:
            df_new_timeseries_values.drop(
                columns=added_cols_all, errors="ignore", inplace=True
            )
    ...
    if "pid_to_date" in locals():
        ...
        rep_map = {
            int(pid): pd.Timestamp(dt).normalize() for pid, dt in pid_to_date.items()
        }
    elif getattr(aggregation, "clusterCenterIndices", None):
        # TSAM medoids available: use medoid date per cluster id
        medoid_dates = (
            df_timeseries.resample("1D")
            .first()
            .iloc[aggregation.clusterCenterIndices]
            .index
        )
        rep_map = {
            int(pid): pd.Timestamp(dt).normalize()
            for pid, dt in enumerate(medoid_dates)
        }
    else:
        ...
        rep_map = {
            int(pid): pd.Timestamp(dt).normalize() for pid, dt in first_dates.items()
        }

    # 2) Create cluster_days with dates in BOTH columns (Calliope expects dates)
    cluster_days = (
        matched_indices.resample("1D")
        .first()[["PeriodNum"]]  # daily PeriodNum (numeric)
        .assign(
            PeriodNum=lambda x: x["PeriodNum"].map(rep_map)
        )  # map to representative DATE
    )
    ...
    assignment = cluster_days[
        "PeriodNum"
    ].copy()  # Series: index=all days, value=rep date

    print(
        f"[TSA] successfully applied{' with soft extremes' if soft_prune and extremes_spec else ''}. "
...
def align_by_third_row(
    ...
    target_order = df_headings.iloc[header_row_index].astype(str).str.strip().tolist()
...
def _add_requested_soc_features(
    ...
    def _name(f):
        return f"{f}__{suffix}" if suffix else f

    feature_builders = {
        "soc_activity_MWh": lambda: by_day.apply(lambda x: x.abs().sum()).rename(
            _name("soc_activity_MWh")
        ),
        "soc_charge_MWh": lambda: by_day.apply(lambda x: x.clip(lower=0).sum()).rename(
            _name("soc_charge_MWh")
        ),
        "soc_discharge_MWh": lambda: by_day.apply(
            lambda x: (-x.clip(upper=0)).sum()
        ).rename(_name("soc_discharge_MWh")),
        "soc_net_MWh": lambda: daily_net.rename(_name("soc_net_MWh")),
        "soc_max_charge_rate": lambda: by_day.max().rename(
            _name("soc_max_charge_rate")
        ),
        "soc_max_discharge_rate": lambda: (-by_day.min()).rename(
            _name("soc_max_discharge_rate")
        ),
        "soc_peak10h_charge": lambda: by_day.apply(
            lambda x: x.rolling(10, min_periods=1).sum().max()
        ).rename(_name("soc_peak10h_charge")),
        "soc_peak10h_discharge": lambda: by_day.apply(
            lambda x: (-x).rolling(10, min_periods=1).sum().max()
        ).rename(_name("soc_peak10h_discharge")),
        "soc_discharge_30d": lambda: (
            daily_dis.rolling(30, min_periods=1)
            .sum()
            .rename(_name("soc_discharge_30d"))
        ),
        "soc_discharge_90d": lambda: (
            daily_dis.rolling(90, min_periods=1)
            .sum()
            .rename(_name("soc_discharge_90d"))
        ),
        "soc_energy_debt": lambda: _energy_debt(daily_net).rename(
            _name("soc_energy_debt")
        ),
    }
...
def _collect_extreme_dates(
    ...
    """
    proxy_cols = (
        list(soc_proxy_dict.get("proxy_inputs_to_consider", []))
        if soc_proxy_dict.get("use_soc_proxy")
        else []
    )
    if not proxy_cols:
    ...
    if verbose and all_rows:
        ...
        for dt, rows in sorted(rows_by_date.items()):
            reasons = ", ".join(
                f"{r['feature']}({r['how']}, rank={r['rank']}, val={r['value']:.3g})"
                for r in rows
            )
...
def _daily_soc_features(df: pd.DataFrame, delta_col: str) -> pd.DataFrame:
    ...
    # sustained ramps (10h)
    def peak10h_pos(x):
        return x.rolling(10, min_periods=1).sum().max()
...
def _tsam_medoids_as_dates(
    df_timeseries: pd.DataFrame, aggregation
) -> List[pd.Timestamp]:
...
def _stable_union(
    a: Iterable[pd.Timestamp], b: Iterable[pd.Timestamp]
) -> List[pd.Timestamp]:
    seen = set()
...
def _stack_days(
    df_scaled: pd.DataFrame, hours_per_period: int
...
def _prune_candidates_by_drop_cost(
    D_all_to_pool: np.ndarray, pool_rows: List[int], target_k: int
) -> List[int]:
    """
    ...
    while len(pool_idx) > target_k:
        ...
        order = np.argpartition(D_S, kth=1, axis=1)[
            :, :2
        ]  # indices of two smallest per row (unordered pair)
...
def _build_typical_periods_from_dates(
    df: pd.DataFrame, chosen: Dict[int, pd.Timestamp], hours_per_period: int
) -> pd.DataFrame:
    """
    ...
    for pid, date in sorted(chosen.items()):
        ...
        if len(day) != hours_per_period:
            raise ValueError(
                f"Day {date.date()} has {len(day)} rows; expected {hours_per_period}."
            )
        tmp = day.copy()
