...
def standardised_model_config(params):
    ...
    path_model_config_yaml = os.path.normpath(
        f"SoC_proxy_TSA/model_config/{params['config_yaml_name']}.yaml"
    )

    # auto-config calliope logging

    calliope.set_log_verbosity(
        "ERROR", include_solver_output=params["calliope_full_log"][0]
    )
    ...
    if "horizon_start" in params and "horizon_start" in params:
        calliope_override_dictionary["config.init.subset.timesteps"] = [
    ...
    if "filename_time_varying_parameters" in params:
        calliope_override_dictionary["data_tables.time_varying_parameters.data"] = (
            f"../data_tables/full_horizon/{params['filename_time_varying_parameters']}.csv"
        )
...
def clustered_model_config(params):
    ...
    if "path_cluster_map" in params:
        ...
        raise Exception(
            "The path_to_cluster_csv variable is not set within the params dictionary."
        )
    ...
    add_math_paths_config = {
        "maths_cyclic_constraints": "../custom_mathematics/add_maths_cycle_constraints.yaml"
    }
...
def reconstructured_full_model_config(params):
    ...
    # define horizons
    if "horizon_start" in params and "horizon_start" in params:
        calliope_override_dictionary["config.init.time_subset"] = [
            params["horizon_start"],
            params["horizon_end"],
        ]
    ...
    if "path_to_new_timeseries" in params and params["path_to_new_timeseries"]:
        calliope_override_dictionary["data_tables.time_varying_parameters.data"] = (
            f"../../{params['path_to_new_timeseries']}"
        )
    elif "filename_time_varying_parameters" in params:
