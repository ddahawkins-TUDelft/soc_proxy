...
def calliope_ts_to_pandas(
    source: str, date_range_lower_bound: str = "", date_range_upper_bound: str = ""
):
    # import timeseries
    df_timeseries = pd.read_csv(source, low_memory=False)
    # reformat
    index_header = df_timeseries[df_timeseries.iloc[:, 0].str.contains("techs")].index[
        0
    ]  # identify row containing new header
    df_timeseries.iloc[index_header, 0] = "timesteps"
    df_timeseries.columns = df_timeseries.iloc[index_header]
    df_timeseries = df_timeseries.iloc[(index_header + 3) :].reset_index(
        drop=True
    )  # remove first row containing comment
    ...
    if date_range_lower_bound:
        df_timeseries = df_timeseries[
            df_timeseries["timesteps"] >= date_range_lower_bound
        ]
    if date_range_upper_bound:
        df_timeseries = df_timeseries[
            df_timeseries["timesteps"]
            < (pd.to_datetime(date_range_upper_bound) + pd.Timedelta(days=1))
        ]
...
def extrapolate_ts_from_cluster_map(source_cluster_map, source_original_ts: str):
    ...
    if isinstance(source_cluster_map, str):
        ...
        df.rename(
            columns={df.columns[0]: "timesteps", df.columns[1]: "PeriodNum"},
            inplace=True,
        )
    else:
    ...
    df = pd.merge(df, df_original_ts, how="left", left_on="PeriodNum", right_on="date")

    # carry the time allocation from the aggregated date-time column to the original date column
    df["timesteps"] = pd.to_datetime(
        df["timesteps_x"].astype(str) + " " + df["timesteps_y"].dt.time.astype(str)
    )
...
def convert_to_hour_of_year(timestamp_series):
    ...
    hour_of_year = (
        dt - pd.Timestamp(year=dt.dt.year.min(), month=1, day=1)
...
def extract_soc_from_clustered_model(
    results_path, ref_model, n_days, method, rep_method, soc_proxy: bool = False
):
    ...
    cluster_map = cluster_map.rename(
    ...
    df_result["full_timestamp"] = df_result[
        "datesteps"
...
def get_capacities(calliope_model, type: str = "standard"):
    ...
    # process SoC
    df_state_of_charge = df_state_of_charge[
        df_state_of_charge["techs"] == "h2_salt_cavern"
    ]
    ...
    ldes_capacity = df_storage_caps.loc[
        df_storage_caps["techs"] == "h2_salt_cavern", "storage_cap"
    ].iloc[0]  # storage cap
    capacities = np.array(
        df_energy_caps.loc[df_energy_caps["carriers"] == "power", "flow_cap"]
    )  # array of power caps
