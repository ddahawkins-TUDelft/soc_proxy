...
def compare_models(
    ...
    df_reference_storage_capacities.replace(
        ["battery", "h2_salt_cavern"], ["SDES_energy", "LDES_energy"], inplace=True
    )
    ...
    df_test_storage_capacities.replace(
        ["battery", "h2_salt_cavern"], ["SDES_energy", "LDES_energy"], inplace=True
    )
    ...
    df_reference_technology_capacities = df_reference_technology_capacities[
        df_reference_technology_capacities["carriers"] == "power"
    ]
    df_reference_technology_capacities = df_reference_technology_capacities[
        df_reference_technology_capacities["techs"] != "demand_power"
    ]
    df_reference_technology_capacities.drop(["nodes", "carriers"], axis=1, inplace=True)
    df_reference_technology_capacities.replace(
        ["battery", "h2_elec_conversion"], ["SDES_power", "LDES_power"], inplace=True
    )
    df_reference_technology_capacities.set_index("techs", inplace=True)
    df_reference_technology_capacities = pd.concat(
        [df_reference_technology_capacities, df_reference_storage_capacities]
    )
    ...
    df_test_technology_capacities = df_test_technology_capacities[
        df_test_technology_capacities["carriers"] == "power"
    ]
    df_test_technology_capacities = df_test_technology_capacities[
        df_test_technology_capacities["techs"] != "demand_power"
    ]
    df_test_technology_capacities.drop(["nodes", "carriers"], axis=1, inplace=True)
    df_test_technology_capacities.replace(
        ["battery", "h2_elec_conversion"], ["SDES_power", "LDES_power"], inplace=True
    )
    df_test_technology_capacities.set_index("techs", inplace=True)
    df_test_technology_capacities = pd.concat(
        [df_test_technology_capacities, df_test_storage_capacities]
    )

    # isolate a single concatenated dataframe
    df_tech_capacities = df_reference_technology_capacities.merge(
        right=df_test_technology_capacities, how="left", on="techs"
    )
    df_tech_capacities["error"] = (
        df_tech_capacities["reference"] - df_tech_capacities["test"]
    )
    df_tech_capacities["error_absolute"] = np.abs(df_tech_capacities["error"])
    df_tech_capacities["error_absolute_normalised"] = np.abs(
        df_tech_capacities["error"] / df_tech_capacities["reference"]
    )

    result = {
        "df_capacities": df_tech_capacities,
        "capacity_metrics": {
            "mean_absolute_capacity_error": np.mean(
                df_tech_capacities["error_absolute_normalised"]
            ),
            "ldes_capacity_error": df_tech_capacities.loc["LDES_energy"][
                "error_absolute_normalised"
            ],
        },
    }
    ...
    if not df_clustermap_test_model.empty:
        df_clustermap_test_model = df_clustermap_test_model.rename(
            columns={"timesteps": "datesteps", "PeriodNum": "mapped_datesteps"}
        )
        df_clustermap_test_model["datesteps"] = pd.to_datetime(
            df_clustermap_test_model["datesteps"], format="%Y-%m-%d"
        )
        df_clustermap_test_model["mapped_datesteps"] = pd.to_datetime(
            df_clustermap_test_model["mapped_datesteps"], format="%Y-%m-%d"
        )
        ...
        df_intracluster_soc = df_intracluster_soc[
            df_intracluster_soc["techs"] == "h2_salt_cavern"
        ]
        df_intracluster_soc["mapped_datesteps"] = pd.to_datetime(
            df_intracluster_soc["timesteps"], format="%Y-%m-%d"
        )
        ...
        df_intercluster_soc = df_intercluster_soc[
            df_intercluster_soc["techs"] == "h2_salt_cavern"
        ]
        df_intracluster_soc["mapped_datesteps"] = df_intracluster_soc[
            "mapped_datesteps"
        ].dt.normalize()

        # merge everything and filter
        df_test_soc = df_intercluster_soc.merge(
            df_clustermap_test_model, on="datesteps", how="left"
        )
        df_test_soc = df_test_soc.merge(
            df_intracluster_soc, on="mapped_datesteps", how="left"
        )
        ...
        df_test_soc["full_timestamp"] = df_test_soc[
            "datesteps"
        ].dt.normalize() + pd.to_timedelta(time_only.astype(str))
        ...
        df_test_soc.drop(
            ["datesteps", "timesteps", "inter_soc", "intra_soc"], axis=1, inplace=True
        )
    ...
    e_time_full_charge, e_time_full_discharge = peak_timing_error(
        y_true,
        y_pred,
    )
    ...
    result["soc_metrics"] = {
        ...
        "first_derivative_correlation": first_derivative_correlation(
            y_true, y_pred, standardise=True
        ),
    ...
    df_reference_capex.replace(
        ["battery", "h2_elec_conversion", "h2_salt_cavern"],
        ["SDES_power", "LDES_power", "LDES_energy"],
        inplace=True,
    )
    df_reference_capex = df_reference_capex[
        df_reference_capex["techs"] != "demand_power"
    ]
    ...
    df_capex["error_absolute_normalised_total_reference_cost"] = df_capex[
        "error_absolute"
    ] / np.sum(df_capex["reference"])
    df_capex["error_absolute_normalised"] = np.abs(
        df_capex["error"] / df_capex["reference"]
    )
    ...
    if proxy_parameters:
        df_reference_soc_proxy = (
            model_reference.inputs[
                [
                    k
                    for k, v in model_reference.inputs.data_vars.items()
        ...
        df_reference_soc_proxy.columns = [
            col[1] if isinstance(col, tuple) else col
            for col in df_reference_soc_proxy.columns
        ]
        ...
        df_reference_soc_proxy.drop(
            columns=["onshore_wind", "demand_power", "offshore_wind", "solar"],
            inplace=True,
        )
        ...
        # combine
        df_soc_proxies = df_reference_soc_proxy.merge(
            df_test_soc_proxy,
            how="left",
            on="timesteps",
...
def peak_timing_error(y_true, y_pred, field_reference: str = "index"):
    ...
    id_trough_true = (
        y_true.idxmin()
    )  # get id of true profile peak, representing the hour
...
def extract_timeseries_from_calliope(model: calliope.Model):
    ...
    raw_data.columns = [
        col[1] if isinstance(col, tuple) else col for col in raw_data.columns
    ]
