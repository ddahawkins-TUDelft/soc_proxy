...
def _load_reference_timeseries_df(
    timeseries_csv_path: Optional[Path], ts_window: List[str]
) -> pd.DataFrame:
    if timeseries_csv_path is None:
        raise FileNotFoundError(
            "timeseries_csv_path not provided. Please pass the reference timeseries CSV used to build the model."
        )
...
def _load_soc_proxy_params(
    model, explicit_params: Optional[Dict[str, Any]]
    ...
    if (
        ...
        and isinstance(model.params, dict)
        and "soc_proxy_params" in model.params
    ):
    ...
    try:
        ...
        pass
    raise KeyError(
        "Could not find 'soc_proxy_params' on the model. Pass soc_proxy_params=... explicitly."
    )


def plot_soc_actual_vs_proxy(
