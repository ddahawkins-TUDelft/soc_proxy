...
def _expand_clustered_intra_series(
    model: CalliopeModel,
    cluster_map_csv: Path,
    tech: str,
    candidates: List[str],
...
def _clustered_soc_series(
    model: CalliopeModel, cluster_map_csv: Path, storage_tech: str
) -> pd.Series:
    ...
    df_inter = (
        model.results["storage_inter_cluster"]
        .fillna(0)
        .to_series()
        .dropna()
        .to_frame("inter")
...
def get_series_for_models(
    ...
    solar_candidates = [
        ...
        "flow_out",
        "flow",
    ]  # fallbacks for different Calliope versions
    solar_c = _expand_clustered_intra_series(
        m_cluster,
        cluster_map_csv,
        tech=solar_tech,
        candidates=solar_candidates,
        carriers=["power"],
        value_name="solar",
    )
    ...
    # SoC
    soc_c = _clustered_soc_series(
        m_cluster, cluster_map_csv=cluster_map_csv, storage_tech=storage_tech
    )
...
def plot_cluster_vs_reference_timeseries(
    cluster_id: str,
    reference_nc_path: Path = Path(
        "SoC_proxy_TSA/data/calliope_models/standard_2016_2017_reference.nc"
    ),
    base_path: Path = Path("SoC_proxy_TSA/data"),
    storage_tech: str = "h2_salt_cavern",
    solar_tech: str = "solar",
    ...
    ax1.plot(
        solar_c.index, solar_c.values, label=f"Clustered ({cluster_id})", linestyle="-"
    )
    ...
    ax2.plot(
        soc_c.index, soc_c.values, label=f"Clustered ({cluster_id})", linestyle="-"
    )
