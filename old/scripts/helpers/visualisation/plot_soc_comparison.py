...
def figure_compare_SoC(
    df_reference, df_test, label_graph_1="Reference", label_graph_2="Test"
):
...
def figure_compare_muliple_SoCs(
    df_reference, list_of_test_dfs, label_graph_1, list_of_test_labels
):
    ...
    for x in range(len(list_of_test_dfs)):
        plt.plot(
            list_of_test_dfs[x].index,
            list_of_test_dfs[x]["soc_proxy"],
            label=list_of_test_labels[x],
            color=colours[x % len(colours)],
            zorder=1,
        )

    plt.plot(
        df_reference.index,
        df_reference["soc_proxy"],
        label=label_graph_1,
        color="black",
        zorder=101,
    )
